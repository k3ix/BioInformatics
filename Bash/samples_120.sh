...
#do
	...
	#gunzip *.gz
	#bowtie2 --local -p $proc -x $dir/GRCh37_by_hand -1 *R1_001.fastq -2 *R2_001.fastq | samtools view -@ $proc -bS | samtools sort -@ $proc -o ${name}_sorted.bam
...
#echo "works"
#cd $dir
for j in {136..190} #$(ls */ -d | grep -v "_" | grep -v "results")
do
	echo $j
	cd $dir/$j
	name=$(pwd | rev | cut -d "/" -f1 | rev)
	...
	freebayes-parallel $dir/GRCh37_regions_freeb.regions $proc -f $ref ${name}.bam > $name.vcf
...
/home/yahor/OsteoPanel_input/FASTQ/GRCh37_by_hand.fna
